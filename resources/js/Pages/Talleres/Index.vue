                ...
                <tr v-for="taller in talleres" :key="taller.id" class="hover:bg-gray-50 transition-colors">
                  ...
                  <td class="px-6 py-4 text-center">
                    <span :class="['text-xs font-semibold px-2.5 py-1 rounded-full', taller.active ? 'bg-green-100 text-green-700' : 'bg-gray-100 text-gray-500']">
                      {{ taller.active ? 'Active' : 'Inactive' }}
                    </span>
                  </td>
                  <td class="px-6 py-4 text-center">
                    <div class="flex items-center justify-center gap-2">
                      <button @click="abrirModal(taller)" class="text-xs text-didasa-red hover:underline font-medium">Edit</button>
                      <span class="text-gray-300">|</span>
                      <button @click="eliminar(taller)" class="text-xs text-gray-400 hover:text-red-600 hover:underline font-medium">Delete</button>
                    </div>
                  </td>
                </tr>
              </tbody>
    ...
    <div v-if="modal.open" class="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm">
      <div class="bg-white rounded-2xl shadow-xl w-full max-w-md mx-4">
        <div class="px-6 py-4 border-b border-gray-100 flex items-center justify-between">
          <h3 class="font-bold text-didasa-black">{{ modal.editando ? 'Edit Workshop' : 'New Workshop' }}</h3>
...
function eliminar(taller) {
  if (confirm(`¿Eliminar "${taller.name}"?`)) {
    router.delete(`/talleres/${taller.id}`)
  }
}
</script>
